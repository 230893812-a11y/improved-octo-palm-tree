...
function normalizeQuestion(question) {
  ...
  return question
    ...
    .replace(/独立设计或独立实现/g, "负责或参与设计或实现")
    .replace(/独立开发或独立实现/g, "负责或参与开发或实现")
    .replace(/独立设计\/独立实现/g, "负责或参与设计/实现")
    .replace(/独立开发\/独立实现/g, "负责或参与开发/实现")
    .replace(/独立设计/g, "负责或参与设计")
    .replace(/独立实现/g, "负责或参与实现")
    .replace(/独立开发/g, "负责或参与开发")
    .replace(/独立承担/g, "负责或参与")
    .replace(/独立主导/g, "负责或参与")
    .replace(/独自完成/g, "负责或参与")
    .replace(/独自负责/g, "负责或参与")
    .replace(/独自主导/g, "负责或参与")
...
async function callDeepSeek(modelInput) {
  ...
  const systemPrompt = [
    ...
    "不得诱导用户把协作成果包装成独立完成、独立负责、独立设计、独立实现、独立开发或独自主导。追问应询问用户具体负责或参与的步骤、交付物和验证结果。",
